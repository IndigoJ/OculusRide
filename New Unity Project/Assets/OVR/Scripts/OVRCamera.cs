...
public class OVRCamera : OVRComponent
{
	...
	{	
		...
		{			
			...
			{
				Vector3 a = camera.transform.localRotation.eulerAngles;
				a.x = 0; 
				a.z = 0;
...
//								    CameraPosition.x, CameraPosition.y, CameraPosition.z));
		...
		foreach(OVRCameraGameObject obj in CameraLocalSetList)
		{
			if(obj.CameraController.GetCameraDepth() == camera.depth)
			{
				...
				obj.CameraGameObject.transform.localRotation = qp;
			}
		}

		// Adjust camera position with offset/clipped cam location
		camera.transform.localPosition += Quaternion.Inverse(camera.transform.parent.localRotation) * qp * newCamPos;
